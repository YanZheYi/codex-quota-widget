...
<p align="center">
  ...
  专为 ChatGPT 开发者、重度 API 使用者打造，提供全天候无感的<b>额度监控、速率重置倒计时与多账户快速热切换</b>。
</p>
...
- 🎨 **极简拟物风设计**：无系统边框，精美的毛玻璃渐变背景，完美适配 Windows 10/11 桌面。
- 📏 **自适应窗口大小**：采用 `ResizeObserver` 动态监听前端布局尺寸。无论是展开设置面板还是显示错误日志，操作系统窗口都会精准贴合卡片尺寸，彻底告别传统网页的“点击穿透”和“操作遮挡”痛点。
- 📊 **双维度额度同步**：
  - **动态环形进度条**：直观展示 5 小时速率限制的额度使用状况，配有流畅的Ease缓动数值增长动画。
  - **高对比重置时间**：显示 5 小时与 7 天限额的精确重置时间，精确到分钟。
- 🔄 **多账号无缝热切换**：刷新时自动重新加载磁盘配置文件，原生支持 JWT 解码，底栏即时渲染当前登录的**用户名称与邮箱**。
- ⚡ **防重复抖动保护**：手动刷新按钮具备 **3 秒冷却保护**及加载状态动画，有效避免频繁调用 API 导致额度滥用或触发限流。
- 📦 **绿色轻量化**：打包生成的单文件 `.exe` 仅约 **6.6MB**，无任何残留或后台常驻进程。
- 🔒 **直连与隐私安全**：请求直接通过 Tauri 客户端底层直连 OpenAI 官方接口，不经过任何第三方代理或云中转，绝对保障您的账户 Token 隐私。
...
> 输入的源图片**必须是完美的正方形**（即长与宽的像素比例必须是 1:1，例如 512x512 或 1024x1024）。如果长宽不相等，构建命令行将抛出 `Source image must be square` 并恐慌退出。
...
- 接口解析与安全通信逻辑启发自开源社区逆向工程实践，特此感谢。
- 本项目仅供学习与技术交流使用，使用非官方 API 存在极低概率的账号风控风险，请知悉。
